...
class Faroswap {
  constructor() {
    ...
    this.tickers = ["PHRS", "WPHRS", "USDC", "USDT", "WETH", "WBTC"];
  ...
  async performDeposit(web3, privKey, address) {
    ...
    try {
      ...
      this.log(
        `${chalk.cyan("     余额 :")} ${chalk.white(
          Web3.utils.fromWei(balance, "ether")
        )} PHRS`
      );
      this.log(
        `${chalk.cyan("     金额 :")} ${chalk.white(this.deposit_amount)} PHRS`
      );
      if (
        parseFloat(Web3.utils.fromWei(balance, "ether")) < this.deposit_amount
      ) {
        this.log(
          `${chalk.cyan("     状态 :")}${chalk.yellow(" PHRS 余额不足")}`
        );
        return;
      }
      const contract = new web3.eth.Contract(
        this.ERC20_CONTRACT_ABI,
        this.WPHRS_CONTRACT_ADDRESS
      );
      ...
      this.log(
        `${chalk.cyan("     区块  :")}${chalk.white(receipt.blockNumber)}`
      );
      this.log(
        `${chalk.cyan("     哈希  :")}${chalk.white(receipt.transactionHash)}`
      );
      this.log(
        `${chalk.cyan("     浏览器:")}${chalk.white(
          `https://testnet.pharosscan.xyz/tx/${receipt.transactionHash}`
        )}`
      );
    } catch (e) {
  ...
  async performWithdraw(web3, privKey, address) {
    ...
    try {
      ...
      this.log(
        `${chalk.cyan("     金额 :")} ${chalk.white(
          this.withdraw_amount
        )} WPHRS`
      );
      if (balanceNum < this.withdraw_amount) {
        this.log(
          `${chalk.cyan("     状态 :")}${chalk.yellow(" WPHRS 余额不足")}`
        );
        return;
      }
      ...
      if (from_token !== this.PHRS_CONTRACT_ADDRESS) {
        ...
        const allowance = BigInt(
          await contract.methods
            .allowance(address, this.MIXSWAP_ROUTER_ADDRESS)
            .call()
        );
        if (allowance < approveAmount) {
          const approveData = contract.methods
            .approve(
              this.MIXSWAP_ROUTER_ADDRESS,
          ...
          this.log(
            `${chalk.cyan("     状态  :")}${chalk.green(" Approve 成功 ")}`
          );
        }
      }
      this.log(
        `${chalk.yellow(
          "     兑换功能请根据实际 DODO 路由 API 实现 calldata、to、value、gasLimit"
        )}`
      );
    } catch (e) {
    ...
    try {
      for (const token of [from_token, to_token]) {
        if (token !== this.PHRS_CONTRACT_ADDRESS) {
          ...
          const allowance = BigInt(
            await contract.methods
              .allowance(address, this.POOL_ROUTER_ADDRESS)
              .call()
          );
          if (allowance < approveAmount) {
            const approveData = contract.methods
              .approve(
                this.POOL_ROUTER_ADDRESS,
                "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"
              )
              .encodeABI();
            const gas = await contract.methods
      ...
      this.log(
        `${chalk.yellow("     添加流动性功能请根据实际合约 ABI 实现参数")}`
      );
    } catch (e) {
      this.log(
        `${chalk.cyan("     状态  :")}${chalk.red(" 添加流动性失败 ")}${e}`
      );
    }
    await this.sleep(this.min_delay);
  }
  ...
  async main() {
    ...
    let proxies = [];
    try {
      proxies = (await fs.readFile("proxy.txt", "utf-8"))
        .split("\n")
        .map((line) => line.trim());
    } catch {}
    ...
    for (let i = 0; i < accounts.length; i++) {
      ...
      this.log(
        chalk.cyan("处理账户: ") +
          chalk.white(this.maskAccount(address)) +
          (proxy ? chalk.yellow(" [代理]") : chalk.gray(" [直连]"))
      );
  ...
  while (true) {
    ...
    while (seconds > 0) {
      ...
      process.stdout.write(
        `\r${chalk.cyan("[等待下轮执行]")} 剩余时间: ${chalk.yellow(
          `${h}:${m}:${s}`
        )}`
      );
